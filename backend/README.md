...
- **.env not found:** Ensure `.env` is in `backend/` and filled out. Use `/test-env` endpoint to debug.
...
- **CORS errors:** Update allowed origins in `app.py` for production.
...
## API Reference
See [about-api.md](about-api.md) for detailed backend API usage and examples.